...
exports.up = function (knex) {
  return knex.schema.table('commandes', function (table) {
    // Statut du paiement
    table
      .enum('statut_paiement', ['impaye', 'paye', 'rembourse'])
      .notNullable()
      .defaultTo('impaye')
    ...
    table
      .string('stripe_checkout_session_id', 255)
      .nullable()
      .comment('ID de la session Stripe Checkout');

    // ID de l\'intent de paiement Stripe
    table.string('stripe_payment_intent_id', 255).nullable().comment('ID du Payment Intent Stripe');
