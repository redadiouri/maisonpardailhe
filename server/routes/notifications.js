...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!adminTo) {
      logger.warn(
        'No admin email configured for notifications (set ADMIN_NOTIFICATION_EMAIL or FROM_ADDRESS)'
      );
    } else {
      const html = `<p>Nouveau message de contact</p><ul><li><strong>Nom:</strong> ${escapeHtml(
        fullname
      )}</li><li><strong>Email:</strong> ${escapeHtml(
        email
      )}</li><li><strong>Objet:</strong> ${escapeHtml(subject || '')}</li></ul><p>${escapeHtml(
        message
      )}</p>`;
      ...
      try {
        emailResult = await emailer.sendMail({
          to: adminTo,
        ...
        if (emailResult && emailResult.previewUrl) {
          logger.info(
            ...
            'Notification email sent (preview available)'
          );
