...
function validateCommande(data) {
  ...
  if (date < today)
    return { ok: false, error: "La date ne peut pas être antérieure à aujourd'hui." };
  ...
  if (!/^\d{2}:\d{2}$/.test(String(data.creneau)))
    return { ok: false, error: 'Format du créneau invalide (hh:mm).' };
  ...
  const rangesForDay =
    locationSettings[location].ranges && locationSettings[location].ranges[wk]
      ? locationSettings[location].ranges[wk]
      : [];
  ...
  if (rangesForDay && rangesForDay.length > 0) {
    if (!isAllowedInRanges(rangesForDay))
      return {
        ok: false,
        error: "Le créneau choisi n'est pas disponible pour le jour et le lieu sélectionnés."
      };
  } else {
    const minM = toMinutes(locMin);
    const maxM = toMinutes(locMax);
    if (cMinutes === null || cMinutes < minM || cMinutes >= maxM)
      return { ok: false, error: "Le créneau choisi n'est pas disponible pour le lieu." };
  }

  const now = new Date();
  const dateIsToday = (function (a, b) {
    return (
  ...
  if (dateIsToday) {
    ...
    if (cMinutes < roundedMinutes)
      return { ok: false, error: "Le créneau doit être ultérieur à l'heure actuelle." };
  }
  ...
  async (req, res) => {
      ...
      try {
        ...
        for (const it of data.items) {
          ...
          const [rows] = await conn.execute(
            'SELECT stock, price_cents FROM menus WHERE id = ? FOR UPDATE',
            [menuId]
          );
          const row = rows[0];
          if (!row) {
          ...
          if (row.stock < qty) {
            ...
            return res
              .status(409)
              .json({ message: 'Stock insuffisant', menu_id: menuId, available: row.stock });
          }
        ...
        (async () => {
          try {
            ...
            logger.error(
              'Failed to send creation email for commande %s: %o',
              id,
              e && (e.stack || e)
            );
          }
    ...
    try {
      const normalized = Object.assign({}, data, {
        date_retrait: normalizeToYMD(data.date_retrait) || data.date_retrait
      });
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const out = {
      ...
      datetime_retrait_display: formatForDisplay(
        cmd.date_retrait && cmd.creneau
          ? `${cmd.date_retrait}T${cmd.creneau}:00`
          : cmd.date_retrait,
        true
      ),
      creneau: cmd.creneau,
