...
router.post('/', auth, async (req, res) => {
  ...
  if (reference.length > 100) return res.status(400).json({ message: 'Référence trop longue (max 100).' });
  if (!Number.isFinite(quantity) || quantity < 0) return res.status(400).json({ message: 'Quantité invalide.' });
  try {
...
router.put('/:id', auth, async (req, res) => {
  ...
  if (data.reference !== undefined) {
    ...
    if (r.length > 100) return res.status(400).json({ message: 'Référence trop longue (max 100).' });
    updates.reference = r;
  }
  if (data.quantity !== undefined) {
    const q = Number(data.quantity);
    if (!Number.isFinite(q) || q < 0) return res.status(400).json({ message: 'Quantité invalide.' });
