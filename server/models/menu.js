...
const Menu = {
  ...
  update: async (id, data) => {
    ...
    if (data.is_quote !== undefined) {
      fields.push('is_quote = ?');
      values.push(data.is_quote ? 1 : 0);
    }
    ...
    if (data.visible_on_menu !== undefined) {
      fields.push('visible_on_menu = ?');
      values.push(data.visible_on_menu ? 1 : 0);
    }
  ...
  getAll: async (visibleOnly = true) => {
    if (visibleOnly) {
      const [rows] = await db.execute(
        `SELECT * FROM menus WHERE visible_on_menu = 1 ORDER BY name ASC`
      );
      return rows;
    }
  ...
  getByIds: async (ids) => {
    ...
    const uniq = Array.from(
      new Set(ids.map((i) => Number(i)).filter((n) => !Number.isNaN(n) && n > 0))
    );
    if (uniq.length === 0) return {};
    const placeholders = uniq.map(() => '?').join(',');
    const [rows] = await db.execute(
      `SELECT id, name, price_cents FROM menus WHERE id IN (${placeholders})`,
      uniq
    );
    const map = {};
