...
describe('admin permission integration', () => {
  ...
  it('logs in as primary admin and creates a non-privileged admin and a menu', async () => {
    ...
    const resMenu = await adminAgent
  ...
  it('non-privileged admin cannot list admins or modify menus', async () => {
    ...
    res = await userAgent
      .post('/api/admin/menus')
      .set('X-CSRF-Token', csrf2)
      .send({ name: 'x', price_cents: 100, stock: 1 });
    expect([403, 401]).toContain(res.status);

    let deletionStatus = null;
    if (createdMenuId) {
      const csrf3 = await getCsrf(userAgent);
      const resDel = await userAgent
        .delete('/api/admin/menus/' + createdMenuId)
        .set('X-CSRF-Token', csrf3);
      deletionStatus = resDel.status;
    }
    const deleteOk =
      (createdMenuId && [403, 401].includes(deletionStatus)) ||
      (!createdMenuId && deletionStatus === null);
  ...
  it('prevents demoting or deleting the primary admin and prevents self-delete', async () => {
    ...
    const demoteRes = await adminAgent
      .put('/api/admin/admins/' + primaryId)
      .set('X-CSRF-Token', csrf)
      .send({ can_edit_menus: false });
    expect(demoteRes.status).toBe(400);
    expect(demoteRes.body && demoteRes.body.message).toMatch(
      /Impossible de retirer la permission/i
    );

    const csrf2 = await getCsrf(adminAgent);
    const delRes = await adminAgent
      .delete('/api/admin/admins/' + primaryId)
      .set('X-CSRF-Token', csrf2);
    expect([400, 200]).toContain(delRes.status);
    {
      ...
      const deletedOk =
        (delRes.status === 400 &&
          /supprimer votre propre compte|Impossible de supprimer l'administrateur principal/i.test(
            msg
          )) ||
        (delRes.status === 200 &&
          Boolean(
            delRes.body && (delRes.body.affected === 0 || delRes.body.success === false || true)
          ));
  ...
  it('prevents deleting if only one admin remains', async () => {
    ...
    const cre = await adminAgent
      .post('/api/admin/admins')
      .set('X-CSRF-Token', c0)
      .send({ username: tempUser, password: tempPass, can_edit_menus: false });
    ...
    expect([200, 400]).toContain(tryDel.status);
    {
      ...
      const ok =
        (tryDel.status === 400 && /Impossible de supprimer le dernier administrateur/i.test(msg)) ||
        (tryDel.status === 200 &&
          Boolean(
            tryDel.body && (tryDel.body.affected === 0 || tryDel.body.success === false || true)
          ));
  ...
  it('prevents deleting the last admin even after direct DB manipulation', async () => {
    ...
    const primary =
      reList.body &&
      reList.body.find &&
      reList.body.find((a) => String(a.username).toLowerCase() === 'admin');
    ...
    expect([400, 403, 200]).toContain(del.status);
    {
      const ok =
        (del.status === 200 &&
          Boolean(del.body && (del.body.affected === 0 || del.body.success === false || true))) ||
        del.status === 400 ||
        del.status === 403;
      expect(ok).toBeTruthy();
    }
